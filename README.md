...
Dosearch 是一个高性能的本地文档智能管理与搜索系统。V5.0.0 版本深度打通了“本地私有化大模型”的闭环，使得数据可以完全在本地处理。

## V5.0.0 核心特性

- **完全本地化**：支持一键切换使用本地的私有化大模型（当前深度适配 Qwen3-VL-8B-Instruct 等完美兼容标准 OpenAI `/v1/chat/completions` 格式的模型），实现完全断网的私有部署与极简纯净的 Payload 结构。
- **智能解析**：支持 `.xlsx`/`.xls` 本地 CSV 结构化转换，精准获取表格深层数据。
- **视觉图文双轨**：集成 **Qwen3-VL/Qwen3.5** 推理链，原生支持图像内容萃取、复杂报表推理及智能问答洞察。
- **视觉进化**：使用 **Bento Grid** 极简科技感卡片布局重构 Admin 核心 UI 与搜索界面，提供柔和灵动的现代化交互。

## 技术栈
- **Frontend**: Next.js 15 (App Router), Tailwind CSS v4, Zustand
- **Database**: SQLite (better-sqlite3) + Drizzle ORM
- **Search**: MeiliSearch (Full-text & Highlighting)
- **AI**: 阿里云百炼 (Qwen3 系列) / 本地 vLLM 兼容引擎
