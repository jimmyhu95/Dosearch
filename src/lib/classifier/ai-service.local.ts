...
async function chat(
  ...
  const apiMode = getSetting('api_mode', 'public');
...
export async function aiClassifyDocument(
  ...
  // OFD hard intercept
  if (title.toLowerCase().endsWith('.ofd')) {
    return {
      categories: [{ id: 'reimbursement', name: '报销文件', confidence: 1 }],
      keywords: ['电子发票', 'OFD'],
      summary: '系统自动识别为 OFD 电子发票文件',
    };
  }

  const truncated = content.slice(0, 3000);

  let systemPrompt = `你是一个拥有10年经验的资深企业级IT架构师和AI产品专家。请仔细阅读用户提供的文档片段（包含文件名、前几个段落和提取的关键信息），并将其精准归类到以下10个类别之一。
...
请分析以下文档信息，仅返回对应的英文ID：`;
  ...
  if (title.includes('发票')) {
    systemPrompt += '\n\n【重要指令】当前文档名称包含"发票"字样。在进行类别推理时，请给予"报销文件(reimbursement)"类别最高判定权重。除非内容明确为业务通知或规章制度，否则优先将其归类为"报销文件"。';
  }
...
export async function aiDocumentQA(content: string, question: string): Promise<string> {
  try {
    let systemPrompt = '你是一个文档问答助手。请根据用户提供的文档内容回答问题。如果文档中没有相关信息，请明确说明。';

    // 如果内容包含明显的 CSV 特征（如密集的逗号和换行标志），则追加特定提示词
    if (content.includes('--- [表名:') && content.split(',').length > 10) {
      systemPrompt += ' 当前内容是提取自 Excel 的 CSV 数据，请作为数据分析师仔细推理行列关系后再回答。';
    }
